...
class UpdateAccountDocsTest extends RestDocsSupport {
    ...
    class Describe_updateAccount {
        ...
        void success() throws Exception {
            ...
            // when // then
            performDocument(request, accessToken, status().isOk(),
                "[update-account] success", "[response] update-account",
        ...
        void error() throws Exception {
            ...
            // when // then
            performErrorDocument(request, authorization, status().is4xxClientError(),
        ...
        void error1() throws Exception {
            ...
            // when // then
            performErrorDocument(request, accessToken, status().isBadRequest(),
        ...
        void error4() throws Exception {
            ...
            // when // then
            performErrorDocument(request, accessToken, status().isNotFound(),
                "[update-account] 조회된 사용자 정보 없음");
        }
    }

    private void performDocument(UpdateAccountRequest request, String accessToken,
    ...
    private void performErrorDocument(UpdateAccountRequest request, String accessToken,
        ResultMatcher status, String identifier) throws Exception {

        performDocument(request, accessToken, status, identifier, "[response] error",
            fieldWithPath("httpStatus").type(JsonFieldType.NUMBER)
                .description("상태 코드"),
            fieldWithPath("message").type(JsonFieldType.STRING)
                .description("상태 메시지"),
