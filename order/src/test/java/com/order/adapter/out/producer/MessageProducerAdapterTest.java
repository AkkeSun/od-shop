...
class MessageProducerAdapterTest extends IntegrationTestSupport {

    @Autowired
    MessageProducerAdapter adapter;
    @Autowired
    RedisTemplate<String, String> redisTemplate;

    @Nested
    @DisplayName("[sendMessage] 카프카로 메시지를 전송하는 메소드")
    class Describe_sendMessage {
        ...
        @Test
        @DisplayName("[error] 메시지 전송중 오류 발생시 sendMessageFallback 메서드가 실행되고 레디스에 메시지를 저장한다.")
